...
# Spinner, intake and canopy framing distinguish the Python-powered S.4.
loft('Spinner',[(-7.0,.02,.02,0),(-6.8,.26,.26,0),(-6.15,.48,.48,0)],sky,32)
sphere('Chin intake lip',(0,-.62,-5.25),(.49,.27,.85),navy)
sphere('Chin intake opening',(0,-.70,-5.85),(.34,.15,.13),black)
for z,rx,ry in [(-2.0,.44,.48),(-.25,.43,.49)]:
    tube('Canopy frame',[(rx*math.cos(a),1.03+ry*math.sin(a),z) for a in [j*math.pi/16 for j in range(17)]],.025,navy)
tube('Canopy sill left',[(-.52,1,-2.1),(-.56,1,-.6),(-.39,1,.05)],.035,navy)
tube('Canopy sill right',[(.52,1,-2.1),(.56,1,-.6),(.39,1,.05)],.035,navy)
...
for s in [-1,1]:
    ...
    for x in [2,2.55]:
        sphere('Cannon fairing',(s*x,.06,-1.36),(.13,.13,.57),navy)
    tube('Wing fold seam',[(s*4.35,.24,-1.0),(s*4.35,.25,.3),(s*4.35,.12,1.4)],.012,metal)
    tube('Aileron hinge',[(s*4.5,.13,1.45),(s*6.35,.07,1.15)],.012,metal)
...
for side in [-1,1]:
    tube('Main oleo',[(0,0,0),(0,-1.15,.15)],.10,metal)
    tube('Torque link',[(0,-.4,.12),(0,-.72,.32),(0,-.95,.15)],.045,metal)
    cylinder('Main tyre',(0,-1.13,.15),.4,.25,black,rotation=(0,math.pi/2,0))
    cylinder('Wheel hub',(side*.135,-1.13,.15),.19,.025,metal,rotation=(0,math.pi/2,0))
    box('Gear door',(side*.15,-.55,.15),(.07,1.05,.55),sky)
    save_asset('wyvern_gear_l' if side<0 else 'wyvern_gear_r')
