...
const roundsBefore=f.rounds;
assert(fx.fireGun(f));
assert.equal(fx.shots.length,4,'Wyvern fires four wing cannons');
assert.equal(f.rounds,roundsBefore-4,'Each Hispano consumes one round');
const localMuzzles=fx.shots.map(s=>s.position.clone().sub(f.position).applyQuaternion(f.attitude.clone().invert()));
assert.deepEqual(localMuzzles.map(p=>Math.round(p.x*100)),[-255,-200,200,255]);
assert(localMuzzles.every(p=>Math.abs(p.z+2.8)<1e-6),'Shots start at the wing barrels');
console.log('PASS four wing-mounted Hispano muzzle origins and ammunition');
